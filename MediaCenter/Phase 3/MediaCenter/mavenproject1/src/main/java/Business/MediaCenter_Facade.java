...
public class MediaCenter_Facade {
    ...
    public void removerConteudo(int idConteudo) {
        if(!this.gestorConteudos.conteudoDuplicado(idConteudo)){
            Conteudo conteudo = this.colecaoCasa.getConteudo(idConteudo);
            this.colecaoCasa.removerConteudo(idConteudo);
            try {
                Files.deleteIfExists(Paths.get(conteudo.getPath()));
            } catch (IOException ex) {
                Logger.getLogger(MediaCenter_Facade.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
